...
def test_server_configure_loads_schema_defaults(configured_server):
    ...
    assert server.current_parameter_file == "ros_params_sim.yaml"
...
def test_server_save_writes_standalone_parameter_file(configured_server):
    ...
    assert result.success
    saved_yaml = yaml.safe_load((tmp_path / "iii_drone" / "parameter_snapshots" / "standalone.yaml").read_text())
    ros_parameters = saved_yaml["/**"]["ros__parameters"]
    assert ros_parameters["use_sim_time"] is True
